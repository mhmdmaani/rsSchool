...
const detailsUrl = `/rsSchool-1.0-SNAPSHOT/studentDetails.jsp`;

const fetchUrl = '/rsSchool-1.0-SNAPSHOT/student';
const createUrl = '/rsSchool-1.0-SNAPSHOT/student';
const updateUrl ='/rsSchool-1.0-SNAPSHOT/student';
const deleteUrl ='/rsSchool-1.0-SNAPSHOT/student';
const searchUrl = '/rsSchool-1.0-SNAPSHOT/student';
const educationPageUrl = '/rsSchool-1.0-SNAPSHOT/education.jsp';
function onLoad() {
    ...
    return axios
        ...
        .then((res) => {
            res.data.forEach((el) => {
                const {id,name,email,tel,image,education} = el;
                addElement(id, name,tel,email,image,education);
            });
        });
}
...
function onCreate() {
    ...
    return axios.post(createUrl,
        ...
        const {id,name,tel,email,image,education} = res.data;
        addElement(id,name,tel,email,image,education);
        hideForm();
    })
}
...
function onSearch(){
    ...
    return axios.get(searchUrl,{
        ...
        clearContainer();
        res.data.forEach((el) => {
            const {id,name,tel,email,image,education} = el;
            addElement(id, name,tel,email,image,education);
        });
    });
}

function addElement(id, name, tel,email, image,education) {
    var container = document.getElementById('elements');
    ...
    if(education.id ){
        var educationNameTag = document.createElement('a');
        educationNameTag.classList.add('card-text');
        educationNameTag.style.display = 'block';
        educationNameTag.innerHTML = `${education.name}`;
        educationNameTag.setAttribute("href",`${educationPageUrl}?id=${education.id}`)
    }
    ...
    if(education.id){
        cardBody.appendChild(educationNameTag);
    }
...
function displayEditForm(id,name,tel,email,image, education) {
...
function updateElement(id, name,tel,email,image,education){
